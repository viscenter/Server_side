...
<xsl:stylesheet
	...
	<xsl:template
		...
		<p
			...
			<span
				...
				<xsl:choose>
					<xsl:when
						test="child::tei:seg[@type = 'speaker']"> </xsl:when>
					<xsl:otherwise>
						<xsl:text>&#160;&#160;&#160;&#160;</xsl:text>
					</xsl:otherwise>
				</xsl:choose>
	...
	<!-- quotations -->
	<xsl:template
		match="tei:q">“<xsl:apply-templates/>”</xsl:template>
	...
	<xsl:template
		name="addDots">
		<xsl:variable
			name="currentChar">1</xsl:variable>
		<xsl:variable
			name="stringLength">
			<xsl:value-of
	...
	<xsl:template
		...
		<xsl:choose>
			<xsl:when
				test="$currentChar &lt;= string-length($myString)">
				<xsl:call-template
					name="addDotsRecurse">
					<xsl:with-param
						name="currentChar">
						<xsl:value-of
							select="$currentChar + 2"/>
					</xsl:with-param>
					<xsl:with-param
						name="stringLength">
						<xsl:value-of
							select="$stringLength + 1"/>
					</xsl:with-param>
					...
					<xsl:with-param
						...
						<xsl:choose>
							<xsl:when
								test="substring($myString,$currentChar,1) = ' '">
								<xsl:value-of
	...
	<xsl:template
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<!-- CWB's default.  Do we want to keep this in generic TEI handling or not? -->
	...
	<xsl:template
		match="tei:add[@place='supralinear']">
		<span
			class="supralinearText">
			<xsl:apply-templates/>
		</span>
	</xsl:template>
	<xsl:template
	...
	<xsl:template
		match="tei:choice">
		<span
			class="choice">(<xsl:apply-templates
				select="tei:sic"/><xsl:apply-templates
				select="tei:orig"/>
			<xsl:apply-templates
				select="tei:corr"/>)</span>
	</xsl:template>
	<xsl:template
		match="tei:sic">
		<span
			class="sic"><xsl:apply-templates/>[sic]</span>
	...
	<xsl:template
		match="tei:corr">
		<span
			class="corr">&#160;&#160;/&#160;&#160;<xsl:apply-templates/></span>
	...
	<!-- NS:  additions May 14, 2010;  see also template for "tei:supplied" -->
	<xsl:template
		match="tei:rs">
		<xsl:choose>
			<xsl:when
